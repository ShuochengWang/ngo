...
//! let handle = unsafe {
//!     io_uring.timeout(&tp as *const _, 0, TimeoutFlags::empty(), complete_fn)
//! };
//! io_uring.submit_requests();
...
impl IoUring {
    ...
    /// Return the number of I/O completions. The Completions of Cancel do not count in the number.
    ...
    /// safety
    ///
    /// The handle must be generated bt this IoUring instance.
    ...
    // Cancel an I/O request by IoToken.
    // Try to change the state of token to Cancelling and push a Cancel request to the submission queue.
}
